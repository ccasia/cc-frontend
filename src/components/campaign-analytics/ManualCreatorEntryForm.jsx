...
} from 'src/api/manual-creator';

import Iconify from 'src/components/iconify';
import FormProvider, { RHFTextField } from 'src/components/hook-form';

import { setFormState } from 'src/sections/campaign/discover/admin/campaign-analytics/stores/analytics.store';
...
const CreatorOptionResolver = ({ submission, onResolved }) => {
  ...
  useEffect(() => {
    if (!isLoading && creator?.user) {
      ...
      const profileLink = c?.instagramProfileLink || c?.tiktokProfileLink || c?.profileLink;
...
const ManualCreatorEntryForm = forwardRef(
  (
    {
      campaignId,
      editingEntry,
    ...
    useEffect(() => {
      setFormState({ isValid, isFormComplete, isSubmitting });
    }, [isValid, isFormComplete, isSubmitting]);
