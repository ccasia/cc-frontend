...
const useGetTimeToTiktokActivation = ({ granularity, startDate, endDate, creditTiers = [] } = {}) => {
  const url = useMemo(() => {
    const base = endpoints.analytics.timeToTiktokActivation;
  ...
  return useMemo(
    () => ({
      timeToActivation: data?.data?.timeToActivation || [],
...
export default useGetTimeToTiktokActivation;
