...
export default function PitchesPendingModal({ open, onClose }) {
  ...
  const handleGoToCreator = (campaignId, creatorName) => {
    router.push(
      `${paths.dashboard.campaign.adminCampaignDetail(campaignId)}?tab=pitch&creator=${encodeURIComponent(creatorName)}`
    );
    onClose();
  };
                ...
                <Box key={campaign.id}>
                  ...
                  <Stack spacing={1}>
                    {items.map((pitch) => (
                      <Box
                        key={pitch.id}
                        onClick={() => handleGoToCreator(pitch.campaignId, pitch.user?.name || '')}
                        sx={{
