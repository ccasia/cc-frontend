...
export default function DraftsPendingModal({ open, onClose }) {
  const router = useRouter();
  const [page, setPage] = useState(1);

  const { data, isLoading } = useSWR(
    open ? endpoints.dashboard.draftsPending : null,
    fetcher,
  ...
  const handleViewSubmission = (submission) => {
    if (submission.campaign?.submissionVersion === 'v4') {
      ...
      router.push(
        paths.dashboard.campaign.manageCreator(submission.campaignId, submission.userId, {
          tabs: 'submission',
        })
      );
    }
                        ...
                        <Stack direction="row" alignItems="center" spacing={1.5}>
                          ...
                          <Box sx={{ flex: 1 }}>
                            ...
                            <Typography sx={{ fontSize: '0.75rem', color: '#9ca3af' }}>
                              {SUBMISSION_TYPE_LABEL[submission.submissionType?.type] || submission.submissionType?.type} &middot;{' '}
