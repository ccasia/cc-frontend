...
const CreditTierCreate = ({ open, onClose, mutate }) => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      const res = await axiosInstance.post(endpoints.creditTier.create, payload);

      enqueueSnackbar(res?.data?.message || 'Credit tier created successfully');
      mutate();
      reset();
      onClose();
    } catch (error) {
      enqueueSnackbar(error?.response?.data?.message || error?.message || 'Error creating credit tier', {
        variant: 'error',
      });
    }
  });
...
export default CreditTierCreate;

CreditTierCreate.propTypes = {
  open: PropTypes.bool,
