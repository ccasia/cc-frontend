  ...
  const handleInitialActivate = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `/api/campaign/initialActivateCampaign/${campaignId}`,
        formData
      );
      ...
      enqueueSnackbar(error.response?.data?.message || 'Failed to assign campaign to admin', {
        variant: 'error',
      });
    } finally {
      setSubmitting(false);
      ...
      <DialogContent>
        <Stack spacing={3} sx={{ mt: 2, mb: 4 }}>
          <FormControl fullWidth error={!!errors.campaignManagers}>
            ...
            {errors.campaignManagers && <FormHelperText>{errors.campaignManagers}</FormHelperText>}
          </FormControl>
        </Stack>
      </DialogContent>
