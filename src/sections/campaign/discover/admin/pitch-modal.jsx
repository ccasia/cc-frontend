  ...
  const handleDecline = async () => {
    if (isDemoCampaign) {
      const reasonLabel =
        maybeReason === 'others'
          ? 'Other'
          : MAYBE_REASONS.find((reason) => reason.value === maybeReason)?.label;
      const updatedPitch = {
        ...pitch,
        status: 'REJECTED',
        displayStatus: 'REJECTED',
        ...(reasonLabel ? { rejectionReason: reasonLabel } : {}),
            ...
            {isClientRole && adminCommentsText.length > 0 && (
              <Box>
                ...
                >
                  <Typography
                    variant="body1"
                    sx={{ color: '#000000', lineHeight: 1.5, whiteSpace: 'pre-wrap' }}
                  >
                    {adminCommentsText}
                  </Typography>
                </Box>
        ...
        <DialogActions sx={{ pb: 3, px: 3 }}>
          ...
          <Button
            ...
            sx={{
              ...
              color: isApproveLikeDialog ? '#fff' : '#000',
