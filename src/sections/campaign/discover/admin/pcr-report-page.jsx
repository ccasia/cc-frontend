...
const PCRReportPage = ({ campaign, onBack, isClientView = false, onCampaignUpdate }) => {
  ...
  const { data: insightsData, isLoading: loadingInsights } = useSocialInsights(
    postingSubmissions,
    campaignId
  );
  ...
  const {
    ...
    campaign?.id
      ? `/api/campaign/${campaign.id}/trends/engagement-heatmap?platform=All&weeks=6`
      : null,
    async (url) => {
    ...
    const loadPCRData = async () => {
      ...
      try {
        ...
        if (response.data.success && response.data.data.content) {
          ...
          const hasEducatorContent =
            (loadedContent.educatorTitle && loadedContent.educatorTitle.trim() !== '') ||
            (loadedContent.educatorContentStyle &&
              loadedContent.educatorContentStyle.trim() !== '');
          const hasThirdContent =
            (loadedContent.thirdTitle && loadedContent.thirdTitle.trim() !== '') ||
            (loadedContent.thirdContentStyle && loadedContent.thirdContentStyle.trim() !== '');
          const hasFourthContent =
            (loadedContent.fourthTitle && loadedContent.fourthTitle.trim() !== '') ||
            (loadedContent.fourthContentStyle && loadedContent.fourthContentStyle.trim() !== '');
          const hasFifthContent =
            (loadedContent.fifthTitle && loadedContent.fifthTitle.trim() !== '') ||
  ...
  const EngagementRateHeatmap = () => {
    const top5CreatorsPhases = useMemo(() => {
      ...
      if (postSnapshots && postSnapshots.length > 0) {
        ...
        postSnapshots.forEach((snapshot) => {
          ...
          if (!creatorPhaseData.has(userId)) {
            ...
            const submission = filteredSubmissions.find((sub) => sub.id === snapshot.submissionId);
        ...
        const creatorsWithAverages = Array.from(creatorPhaseData.values()).map((creator) => {
          const avgER = creator.snapshotCount > 0 ? creator.overallER / creator.snapshotCount : 0;
      ...
      filteredInsightsData.forEach((insightData, idx) => {
        ...
        console.log(
          `📅 Post ${idx}: ${daysFromStart.toFixed(1)} days from start (${postDate.toISOString()})`
        );
        ...
        if (!phase || daysFromStart < 0 || daysFromStart > campaignDuration) {
          if (daysFromStart < 0) {
            console.log(
              `⚠️ Post ${idx} is BEFORE campaign start (${Math.abs(daysFromStart).toFixed(1)} days before)`
            );
          } else {
            console.log(
              `⚠️ Post ${idx} is AFTER campaign end (${(daysFromStart - campaignDuration).toFixed(1)} days after)`
            );
          }
          return;
        }
        ...
        if (!creatorPhaseData.has(userId)) {
          ...
          const platformUsername =
            submission.platform === 'Instagram' ? instagramHandle : tiktokHandle;
        ...
        if (!Number.isNaN(engagementRate) && engagementRate > 0) {
          ...
          if (!creatorData.firstPostPhase) {
            ...
            console.log(
              `   → Will track this post's ER (${engagementRate.toFixed(2)}%) across all future periods`
            );
          }
        }
      });
      ...
      top5.forEach((c, i) => {
        console.log(
          `${i + 1}. ${c.name}: Overall ER ${c.overallER.toFixed(2)}% (First posted in: ${c.firstPostPhase})`
        );
    ...
    const campaignAvg = useMemo(() => {
      ...
      const creatorAverages = Array.from(allCreatorERs.values())
        .map((creator) => (creator.postCount > 0 ? creator.totalER / creator.postCount : 0))
        .filter((avg) => avg > 0);
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {displayData.map((creator, index) => {
              ...
              if (fetchedCreatorData?.user) {
                ...
                displayName =
                  fetchedCreatorData.user.username ||
                    ...
                    {creator.firstWeek !== null && (
                      <Box
                        sx={{
                          flex: 1,
                          height: '40px',
                          backgroundColor: creator.firstWeek
                            ? getPhaseColor(creator.firstWeek)
                            : '#E5E7EB',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                    ...
                    {creator.midPeriod !== null && (
                      <Box
                        sx={{
                          flex: 1,
                          height: '40px',
                          backgroundColor: creator.midPeriod
                            ? getPhaseColor(creator.midPeriod)
                            : '#E5E7EB',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
                    ...
                    {creator.finalWeek !== null && (
                      <Box
                        sx={{
                          flex: 1,
                          height: '40px',
                          backgroundColor: creator.finalWeek
                            ? getPhaseColor(creator.finalWeek)
                            : '#E5E7EB',
                          display: 'flex',
                          alignItems: 'center',
                          justifyContent: 'center',
  ...
  const PlatformInteractionsChart = () => {
    ...
    const tiktokPercentage =
      platformData.total > 0 ? (platformData.tiktok / platformData.total) * 100 : 0;
    const instagramPercentage =
      platformData.total > 0 ? (platformData.instagram / platformData.total) * 100 : 0;
  ...
  const HighestViewWeekChart = () => {
    const weekViewsData = useMemo(() => {
      ...
      const earliestActualPost =
        actualPostDates.length > 0 ? Math.min(...actualPostDates) : Date.now();
      ...
      const firstPostDate = new Date(
        Math.min(...highestWeek.posts.map((p) => new Date(p.date).getTime()))
      );
      ...
      const distributionPattern = [0.12, 0.15, 0.18, 0.22, 0.15, 0.1, 0.08];
  ...
  const mostViewsUserId =
    typeof mostViewsCreator?.submission?.user === 'string'
      ? mostViewsCreator?.submission?.user
      : mostViewsCreator?.submission?.user?.id;
  const mostCommentsUserId =
    typeof mostCommentsCreator?.submission?.user === 'string'
  ...
  const { data: mostCommentsCreatorData } = useGetCreatorById(
    !isCommentsManual ? mostCommentsUserId : null
  );
  ...
  const TopCreatorViewsChart = () => {
    ...
    const top5Creators = useMemo(() => {
      ...
      return creatorsWithViews.sort((a, b) => b.views - a.views).slice(0, 5);
    }, []);
  ...
  const TopCreatorViews48HChart = () => {
    ...
    const creatorDataList = [
      views48hCreator0Data,
      views48hCreator1Data,
      views48hCreator2Data,
      views48hCreator3Data,
      views48hCreator4Data,
    ].slice(0, creatorIds.length);
  ...
  const TopEngagementCard = () => {
    const top5Creators = useMemo(() => {
      ...
      return creatorsWithEngagement.sort((a, b) => b.engagementRate - a.engagementRate).slice(0, 5);
    }, []);
    ...
    const creatorDataList = [
      engagementCreator0Data,
      engagementCreator1Data,
      engagementCreator2Data,
      engagementCreator3Data,
      engagementCreator4Data,
    ].slice(0, creatorIds.length);

    // Find max engagement rate for bar width calculation
    const maxEngagementRate =
      top5Creators.length > 0 ? Math.max(...top5Creators.map((c) => c.engagementRate)) : 0;
              ...
              return (
                <Box key={index} sx={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    <Box
                      component="img"
                      src={platform === 'Instagram' ? '/assets/Icon copy.svg' : '/assets/Icon.svg'}
                ...
                <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
                  {!sectionVisibility.engagement && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, engagement: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                  ...
                  {!sectionVisibility.platformBreakdown && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, platformBreakdown: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                  ...
                  {!sectionVisibility.audienceSentiment && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, audienceSentiment: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                  ...
                  {!sectionVisibility.creatorTiers && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, creatorTiers: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                  ...
                  {!sectionVisibility.strategies && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, strategies: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                  ...
                  {!sectionVisibility.recommendations && (
                    <Button
                      size="small"
                      onClick={() =>
                        setSectionVisibility({ ...sectionVisibility, recommendations: true })
                      }
                      sx={{
                        textTransform: 'none',
                        bgcolor: '#FFFFFF',
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Engagement section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                          console.error('Error saving section:', error);
                                          enqueueSnackbar('Failed to save section', {
                                            variant: 'error',
                                          });
                                        } finally {
                                          setIsSaving(false);
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Engagement section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                  ...
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.engagementDescription,
                                      }}
                                    />
                                  );
                                }
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Platform Breakdown section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Platform Breakdown section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                    ...
                                    <Box sx={{ position: 'relative', mb: 3 }}>
                                      ...
                                      <FormattedTextField
                                        ...
                                        onChange={(e) =>
                                          setEditableContent({
                                            ...editableContent,
                                            platformBreakdownDescription: e.target.value,
                                          })
                                        }
                                        placeholder="type here"
                                        rows={2}
                                ...
                                if (editableContent.platformBreakdownDescription) {
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.platformBreakdownDescription,
                                      }}
                                    />
                                  );
                                }
                                  ...
                                  <Grid container spacing={2}>
                                    ...
                                    {(() => {
                                      const views = mostLikesCreator
                                        ...
                                        : 0;
                                      const comments = mostLikesCreator
                                        ? getMetricValue(
                                            mostLikesCreator.insightData.insight,
                                            'comments'
                                          )
                                    ...
                                    {(() => {
                                      const views = mostSharesCreator
                                        ...
                                        : 0;
                                      const comments = mostSharesCreator
                                        ? getMetricValue(
                                            mostSharesCreator.insightData.insight,
                                            'comments'
                                          )
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar('Views section saved successfully', {
                                              variant: 'success',
                                            });
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Views section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                    ...
                                    <Box sx={{ position: 'relative', mb: 3 }}>
                                      ...
                                      <FormattedTextField
                                        ...
                                        onChange={(e) =>
                                          setEditableContent({
                                            ...editableContent,
                                            viewsDescription: e.target.value,
                                          })
                                        }
                                        placeholder="type here"
                                        rows={3}
                                ...
                                if (editableContent.viewsDescription) {
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.viewsDescription,
                                      }}
                                    />
                                  );
                                }
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Audience Sentiment section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Audience Sentiment section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                    ...
                                    <Box sx={{ position: 'relative', mb: 3 }}>
                                      ...
                                      <FormattedTextField
                                        ...
                                        onChange={(e) =>
                                          setEditableContent({
                                            ...editableContent,
                                            audienceSentimentDescription: e.target.value,
                                          })
                                        }
                                        placeholder="type here"
                                        rows={3}
                                ...
                                if (editableContent.audienceSentimentDescription) {
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.audienceSentimentDescription,
                                      }}
                                    />
                                  );
                                }
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Creator Tiers section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Creator Tiers section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                      ...
                                      <Box sx={{ position: 'relative' }}>
                                        ...
                                        <Box
                                          ...
                                          onInput={(e) =>
                                            setEditableContent({
                                              ...editableContent,
                                              creatorTiersDescription: e.currentTarget.innerHTML,
                                            })
                                          }
                                          onKeyDown={(e) => {
                                            const isMod = e.metaKey || e.ctrlKey;
                                ...
                                if (editableContent.creatorTiersDescription) {
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.creatorTiersDescription,
                                      }}
                                    />
                                  );
                                }
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Strategies Utilised section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Strategies Utilised section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
                                    ...
                                    <Box sx={{ position: 'relative', mb: 4 }}>
                                      ...
                                      <FormattedTextField
                                        ...
                                        onChange={(e) =>
                                          setEditableContent({
                                            ...editableContent,
                                            bestPerformingPersonasDescription: e.target.value,
                                          })
                                        }
                                        placeholder="type here"
                                        rows={3}
                                ...
                                if (editableContent.bestPerformingPersonasDescription) {
                                  return (
                                    <Box
                                      ...
                                      dangerouslySetInnerHTML={{
                                        __html: editableContent.bestPerformingPersonasDescription,
                                      }}
                                    />
                                  );
                                }
                                          ...
                                          >
                                            ...
                                            <Box
                                              ...
                                              sx={{
                                                ...
                                                borderRadius: '50%',
                                                background:
                                                  'linear-gradient(135deg, #1340FF 0%, #1340FF 100%)',
                                                display: 'flex',
                                                alignItems: 'center',
                                                justifyContent: 'center',
                                                fontSize: '32px',
                                                ...
                                                <Box sx={{ position: 'relative', mb: 0.75 }}>
                                                  ...
                                                  <TextField
                                                    ...
                                                    onChange={(e) =>
                                                      setEditableContent({
                                                        ...editableContent,
                                                        comicContentStyle: e.target.value,
                                                      })
                                                    }
                                                    fullWidth
                                                    multiline
                                                  ...
                                                  <Box sx={{ position: 'relative', mb: 0.75 }}>
                                                    ...
                                                    <TextField
                                                      ...
                                                      onChange={(e) =>
                                                        setEditableContent({
                                                          ...editableContent,
                                                          educatorContentStyle: e.target.value,
                                                        })
                                                      }
                                                      fullWidth
                                                      multiline
                                            ...
                                            >
                                              ...
                                              <Box
                                                ...
                                                sx={{
                                                  ...
                                                  borderRadius: '50%',
                                                  background:
                                                    'linear-gradient(135deg, #FF3500 0%, #FF3500 100%)',
                                                  display: 'flex',
                                                  alignItems: 'center',
                                                  justifyContent: 'center',
                                                  fontSize: '32px',
                                            ...
                                            <Box
                                              sx={{
                                                ...
                                                transform: effectiveEditMode
                                                  ? 'none'
                                                  : 'translateY(-50%)',
                                                maxWidth: effectiveEditMode
                                                  ? 'calc(920px - 82px)'
                                                  : 'calc(860px - 82px)', // Card width minus circle space
                                                transition: 'top 0.3s ease, transform 0.3s ease',
                                                overflow: 'visible',
                                              }}
                                            >
                                                  ...
                                                  <Box sx={{ position: 'relative', mb: 0.75 }}>
                                                    ...
                                                    <TextField
                                                      ...
                                                      onChange={(e) =>
                                                        setEditableContent({
                                                          ...editableContent,
                                                          thirdContentStyle: e.target.value,
                                                        })
                                                      }
                                                      fullWidth
                                                      multiline
                                            ...
                                            >
                                              ...
                                              <Box
                                                ...
                                                sx={{
                                                  ...
                                                  borderRadius: '50%',
                                                  background:
                                                    'linear-gradient(135deg, #D8FF01 0%, #D8FF01 100%)',
                                                  display: 'flex',
                                                  alignItems: 'center',
                                                  justifyContent: 'center',
                                                  fontSize: '32px',
                                                  ...
                                                  <Box sx={{ position: 'relative', mb: 0.75 }}>
                                                    ...
                                                    <TextField
                                                      ...
                                                      onChange={(e) =>
                                                        setEditableContent({
                                                          ...editableContent,
                                                          fourthContentStyle: e.target.value,
                                                        })
                                                      }
                                                      fullWidth
                                                      multiline
                                            ...
                                            >
                                              ...
                                              <Box
                                                ...
                                                sx={{
                                                  ...
                                                  borderRadius: '50%',
                                                  background:
                                                    'linear-gradient(135deg, #026D54 0%, #026D54 100%)',
                                                  display: 'flex',
                                                  alignItems: 'center',
                                                  justifyContent: 'center',
                                                  fontSize: '32px',
                                                  ...
                                                  <Box sx={{ position: 'relative', mb: 0.75 }}>
                                                    ...
                                                    <TextField
                                                      ...
                                                      onChange={(e) =>
                                                        setEditableContent({
                                                          ...editableContent,
                                                          fifthContentStyle: e.target.value,
                                                        })
                                                      }
                                                      fullWidth
                                                      multiline
                                    ...
                                    >
                                      ...
                                      <Box
                                        sx={{
                                          display: 'flex',
                                          flexDirection:
                                            showEducatorCard || showThirdCard ? 'column' : 'row',
                                          alignItems: 'center',
                                          gap: showEducatorCard || showThirdCard ? 0.5 : 2,
                                          flex: 1,
                                        }}
                                      >
                                        ...
                                        <Box
                                          sx={{
                                            ...
                                            alignItems: 'center',
                                            justifyContent: 'center',
                                            flex: showEducatorCard || showThirdCard ? 1 : 'none',
                                          }}
                                        >
                                          {(() => {
                                              ...
                                              return (
                                                <svg width="360" height="360" viewBox="0 0 160 160">
                                                  {(() => {
                                                    const comicCount =
                                                      parseInt(
                                                        editableContent.creatorStrategyCount,
                                                        10
                                                      ) || 1;
                                                    const educatorCount =
                                                      parseInt(
                                                        editableContent.educatorCreatorCount,
                                                        10
                                                      ) || (showEducatorCard ? 1 : 0);
                                                    const thirdCount =
                                                      parseInt(
                                                    ...
                                                    const fourthCount =
                                                      parseInt(
                                                        editableContent.fourthCreatorCount,
                                                        10
                                                      ) || (showFourthCard ? 1 : 0);
                                                    const fifthCount =
                                                      parseInt(
                                                        editableContent.fifthCreatorCount,
                                                        10
                                                      ) || (showFifthCard ? 1 : 0);
                                                    const total =
                                                      comicCount +
                                                    ...
                                                    const comicAngle =
                                                      comicPercentage * 2 * Math.PI;
                                                    const educatorAngle =
                                                      educatorPercentage * 2 * Math.PI;
                                                    const thirdAngle =
                                                      thirdPercentage * 2 * Math.PI;
                                                    const fourthAngle =
                                                      fourthPercentage * 2 * Math.PI;
                                                    const fifthAngle =
                                                      fifthPercentage * 2 * Math.PI;

                                                    let currentAngle = 0;
                                                    const comicEndAngle = currentAngle + comicAngle;
                                                    const comicEndX =
                                                      80 + 70 * Math.sin(comicEndAngle);
                                                    const comicEndY =
                                                      80 - 70 * Math.cos(comicEndAngle);
                                                    const comicLargeArc =
                                                      comicAngle > Math.PI ? 1 : 0;

                                                    currentAngle = comicEndAngle;
                                                    const educatorEndAngle =
                                                      currentAngle + educatorAngle;
                                                    const educatorEndX =
                                                      80 + 70 * Math.sin(educatorEndAngle);
                                                    const educatorEndY =
                                                      80 - 70 * Math.cos(educatorEndAngle);
                                                    const educatorLargeArc =
                                                      educatorAngle > Math.PI ? 1 : 0;

                                                    currentAngle = educatorEndAngle;
                                                    const thirdEndAngle = currentAngle + thirdAngle;
                                                    const thirdEndX =
                                                      80 + 70 * Math.sin(thirdEndAngle);
                                                    const thirdEndY =
                                                      80 - 70 * Math.cos(thirdEndAngle);
                                                    const thirdLargeArc =
                                                      thirdAngle > Math.PI ? 1 : 0;

                                                    currentAngle = thirdEndAngle;
                                                    const fourthEndAngle =
                                                      currentAngle + fourthAngle;
                                                    const fourthEndX =
                                                      80 + 70 * Math.sin(fourthEndAngle);
                                                    const fourthEndY =
                                                      80 - 70 * Math.cos(fourthEndAngle);
                                                    const fourthLargeArc =
                                                      fourthAngle > Math.PI ? 1 : 0;

                                                    currentAngle = fourthEndAngle;
                                                    const fifthEndAngle = currentAngle + fifthAngle;
                                                    const fifthEndX =
                                                      80 + 70 * Math.sin(fifthEndAngle);
                                                    const fifthEndY =
                                                      80 - 70 * Math.cos(fifthEndAngle);
                                                    const fifthLargeArc =
                                                      fifthAngle > Math.PI ? 1 : 0;

                                                    const comicMidAngle = comicAngle / 2;
                                                    const comicTextX =
                                                      80 + 35 * Math.sin(comicMidAngle);
                                                    const comicTextY =
                                                      80 - 35 * Math.cos(comicMidAngle);

                                                    const educatorMidAngle =
                                                      comicAngle + educatorAngle / 2;
                                                    const educatorTextX =
                                                      80 + 35 * Math.sin(educatorMidAngle);
                                                    const educatorTextY =
                                                      80 - 35 * Math.cos(educatorMidAngle);

                                                    const thirdMidAngle =
                                                      comicAngle + educatorAngle + thirdAngle / 2;
                                                    const thirdTextX =
                                                      80 + 35 * Math.sin(thirdMidAngle);
                                                    const thirdTextY =
                                                      80 - 35 * Math.cos(thirdMidAngle);

                                                    const fourthMidAngle =
                                                      comicAngle +
                                                      educatorAngle +
                                                      thirdAngle +
                                                      fourthAngle / 2;
                                                    const fourthTextX =
                                                      80 + 35 * Math.sin(fourthMidAngle);
                                                    const fourthTextY =
                                                      80 - 35 * Math.cos(fourthMidAngle);
                                                    ...
                                                    const fifthMidAngle =
                                                      ...
                                                      fifthAngle / 2;
                                                    const fifthTextX =
                                                      80 + 35 * Math.sin(fifthMidAngle);
                                                    const fifthTextY =
                                                      80 - 35 * Math.cos(fifthMidAngle);
                                                    ...
                                                    return (
                                                      <>
                                                        ...
                                                        <text
                                                          ...
                                                          style={{
                                                            textShadow:
                                                              '1px 1px 3px #231F20, 0px 0px 2px rgba(0, 0, 0, 0.8)',
                                                          }}
                                                        >
                                                          {comicCount}
                                                        </text>
                                              ...
                                              return (
                                                ...
                                                <svg width="360" height="360" viewBox="0 0 160 160">
                                                  {(() => {
                                                    ...
                                                    const comicEndX =
                                                      80 + 70 * Math.sin(comicAngle);
                                                    const comicEndY =
                                                      80 - 70 * Math.cos(comicAngle);
                                                    const comicLargeArc =
                                                      comicPercentage > 0.5 ? 1 : 0;
                                        ...
                                        <Box
                                          sx={{
                                            ...
                                            flexDirection:
                                              showEducatorCard || showThirdCard ? 'column' : 'row',
                                            alignItems: 'center',
                                            gap: showEducatorCard || showThirdCard ? -1 : 2,
                                            flex: 1,
                                          }}
                                        >
                                          {/* Full Circle Chart or Pie Chart */}
                                                  ...
                                                  >
                                                    {(() => {
                                                      ...
                                                      const total =
                                                        ...
                                                        (showFifthCard ? fifthCount : 0);
                                                      const comicPercentage = comicCount / total;
                                                      const educatorPercentage = showEducatorCard
                                                        ? educatorCount / total
                                                        : 0;
                                                      const thirdPercentage = showThirdCard
                                                        ? thirdCount / total
                                                        : 0;
                                                      const fourthPercentage = showFourthCard
                                                        ? fourthCount / total
                                                        : 0;
                                                      const fifthPercentage = showFifthCard
                                                        ? fifthCount / total
                                                        : 0;
                                  ...
                                  <Box sx={{ display: 'flex', gap: 1, ml: 2 }}>
                                      ...
                                      onClick={async () => {
                                        try {
                                          ...
                                          if (response.data.success) {
                                            ...
                                            enqueueSnackbar(
                                              'Recommendations section saved successfully',
                                              { variant: 'success' }
                                            );
                                          }
                                        } catch (error) {
                                    ...
                                    <IconButton
                                      onClick={() => {
                                        ...
                                        enqueueSnackbar('Recommendations section removed', {
                                          variant: 'info',
                                        });
                                      }}
                                      sx={{
