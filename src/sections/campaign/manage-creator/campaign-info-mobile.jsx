...
const CampaignInfoMobile = ({ campaign }) => {
  ...
  const renderAdditionalDetails1 = () => (
    <Stack spacing={2}>
      ...
      {campaign?.campaignBrief?.otherAttachments?.length > 0 && (
        <Box>
          <Typography
            sx={{ ...SectionTitleStyle, overflowWrap: 'anywhere', whiteSpace: 'normal' }}
          >
            Other Attachment{campaign.campaignBrief.otherAttachments.length > 1 ? 's' : ''}
          </Typography>
          <Stack spacing={0.5}>
...
CampaignInfoMobile.propTypes = {
  campaign: PropTypes.object,
};

export default CampaignInfoMobile;
